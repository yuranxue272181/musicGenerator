...
class CNNGenerator(nn.Module):
    def __init__(self, latent_dim=100, output_shape=(4, 128, 500)):
        ...
        # 先映射到 feature map 初始形状（例：16 x 32 x 63）
        ...
        self.conv_blocks = nn.Sequential(
            ...
            nn.Upsample(scale_factor=(1, 2)),  # time 尺度更长
    ...
    def forward(self, z):
        out = self.fc(z)
        out = out.view(z.size(0), *self.init_shape)  # reshape to (B, C, H, W)
        out = self.conv_blocks(out)  # shape: (B, 4, 128, 500)
        return out
...
def train_musegan(midi_dir, epochs=100, batch_size=16, latent_dim=100, fs=100, fixed_length=500, max_tracks=4
    ...
    generator = CNNGenerator(latent_dim=latent_dim, output_shape=sample_shape)
